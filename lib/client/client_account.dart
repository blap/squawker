...
const bool tryToCreateGuestAccount = false;
...
class TwitterAccount {
  ...
  static Future<void> loadAllTwitterTokensAndRateLimits() async {
    ...
    for (int i = 0; i < twitterProfilesDbData.length; i++) {
      TwitterProfileEntity tpe = TwitterProfileEntity(
        ...
        createdAt: convertTwitterDateTime(twitterProfilesDbData[i]['created_at'] as String)!,
    ...
    for (int i = 0; i < twitterTokensDbData.length; i++) {
      TwitterTokenEntity tte = TwitterTokenEntity(
        ...
        createdAt: convertTwitterDateTime(twitterTokensDbData[i]['created_at'] as String)!,
      );

      if (tte.oauthToken.isNotEmpty && tte.oauthTokenSecret.isNotEmpty) {
        if (!tte.guest) {
          TwitterProfileEntity? twitterProfile = _twitterProfileLst.firstWhereOrNull(
            (e) => e.username == tte.screenName,
          );
    ...
    if (_rateLimits.keys.contains('')) {
      MapEntry<String, List<Map<String, int>>>? merl = _rateLimits.entries.firstWhereOrNull(
        (e) => e.key != '' && e.value[0].isEmpty,
      );
      if (merl != null) {
  ...
  static Future<void> announcementRegularAccountAndUnauthenticatedAccess(BuildContext context) async {
    ...
    // Guard the use of BuildContext across async gap
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Wrap(
            children: [
              Text(
                L10n.current.warning_regular_account_unauthenticated_access_description,
          ...
          actions: [TextButton(onPressed: () => Navigator.pop(context), child: Text(L10n.current.ok))],
        );
      },
    );
  ...
  static Future<void> updateRateValues(String uriPath, int remaining, int reset) async {
    ...
    await repository.update(
      tableRateLimits,
      {'remaining': json.encode(rateLimitRemaining), 'reset': json.encode(rateLimitReset)},
      where: 'oauth_token = ?',
  ...
  static Future<String> getAccessToken() async {
    ...
    var response = await AppHttpClient.httpPost(
      ...
      headers: {'Authorization': 'Basic $oauthConsumerKeySecret', 'Content-Type': 'application/x-www-form-urlencoded'},
  ...
  static Future<String> _getSignOauth(Uri uri, String method) async {
    ...
    String signature = Uri.encodeComponent(
      await hmacSHA1('$oauthConsumerSecret&${_currentTwitterToken!.oauthTokenSecret}', toSign),
    );
