...
class TwitterRegularAccount {
  ...
  static Future<Map<String, dynamic>> _alternateIdentifierFlowToken(
    ...
    var response = await AppHttpClient.httpPost(
      Uri.parse('https://api.twitter.com/1.1/onboarding/task.json'),
      headers: headers,
      body: json.encode({
        'flow_token': flowToken,
        'subtask_inputs': [
    ...
    while ((subtask = _findSubtask(res['subtasks'], 'LoginSuccessSubtask')) == null) {
      if ((subtask = _findSubtask(res['subtasks'], 'LoginEnterUserIdentifier')) != null) {
        ...
        if (context != null) {
          ...
          if (!context.mounted) {
            throw TwitterAccountException('Context is no longer mounted for LoginTwoFactorAuthChallenge');
          }
        ...
        if (context != null) {
          ...
          // Check if context is still mounted before using it across async gap
          if (!context.mounted) {
            throw TwitterAccountException('Context is no longer mounted for LoginEnterAlternateIdentifierSubtask');
          }
          String? text = await _askForInput(context, text1!, text2);
          if (text?.isNotEmpty ?? false) {
        ...
        if (context != null) {
          ...
          // Check if context is still mounted before using it across async gap
          if (!context.mounted) {
            throw TwitterAccountException('Context is no longer mounted for LoginAcid');
          }
          String? text = await _askForInput(context, text1!, text2);
          if (text?.isNotEmpty ?? false) {
        ...
        throw TwitterAccountException(
          'Don'
          't know what to do with ${jsonEncode(res['subtasks'])}',
        );
      }
